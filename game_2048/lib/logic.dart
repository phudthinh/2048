...
class Game {
  ...
  Game(this.row, this.column)
      : _boardCells = List.generate(
          row,
          (r) => List.generate(
            column,
            (c) => BoardCell(row: r, column: c, number: 0, isNew: false),
          ),
        ) {
    init();
  }

  final List<List<BoardCell>> _boardCells;

  void init() {
    for (int r = 0; r < row; ++r) {
      for (int c = 0; c < column; ++c) {
        _boardCells[r][c] = BoardCell(
          row: r,
          column: c,
          number: 0,
          isNew: false,
  ...
  void randomEmptyCell(int cnt) {
    List<BoardCell> emptyCells = []; // Explicitly specifying the type here
